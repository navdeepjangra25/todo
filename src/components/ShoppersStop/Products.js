        ...
        ? state.products.map(prod => {
            return (
                <div key={prod.id} style={{display:'flex',flexDirection:'column',padding:10, border:'1px solid grey', width:"30%",marginTop:10,gap:10}}>
                    ...
                    ?
                    <button style={{padding:5, border:0, borderRadius:5,backgroundColor:"red", color:"white"}} onClick={()=>dispatch({
                        type:'REMOVE_FROM_CART',payload:{
                            id:prod.id
                        }
                    })}>Remove from Cart</button>
                    :
                    <button style={{padding:5, border:0, borderRadius:5,backgroundColor:"green", color:"white"}} onClick={()=>dispatch({
